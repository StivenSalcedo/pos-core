<div class="container">
    ...
    <x-commons.table-responsive>
        ...
        <x-slot:top title="Cierres de caja">

            <x-commons.tag tooltip="Total propinas" label="Propinas" value="{{ formatToCop($totales->tips) }}" />

            <x-commons.tag tooltip="Total de egresos" label="Egresos" value="{{ formatToCop($totales->outputs) }}" />

            <x-commons.tag tooltip="Total de ventas" label="Ventas" value="{{ formatToCop($totales->total_sales) }}" />
        ...
        <x-slot:header>
            ...
            <x-wireui.native-select label="Estado del cierre" optionKeyValue wire:model="confirmedFilter" class="w-56"
                :options="['confirmed' => 'Confirmados', 'unconfirmed' => 'No confirmados', 'all' => 'Todos']" />
                ...
                @forelse ($closings as $item)
                    <tr wire:key="cash-closing-{{ $item->id }}">
                        ...
                        @if ($item->price - $item->cash_register >= 0)
                            <td class="text-green-600" left>
                                @formatToCop($item->price - $item->cash_register)
                            </td>
                        @else
                            <td class="text-red-600" left>
                                @formatToCop($item->price - $item->cash_register)
                            </td>
                        @endif
                        ...
                        <td actions>
                            <x-buttons.icon icon="pdf" class="text-red-600" :href="route('admin.cash-closing.pdf', $item->id)" target="_blank"
                                title="Descargar PDF" text="Cerrar caja" />
                            ...
                            @if (is_null($item->confirmed_at))
                                <x-buttons.icon icon="check"
                                    wire:click="$emitTo('admin.cash-closing.show', 'openShow', {{ $item->id }})"
                                    text="Cerrar caja" title="Confirmar" />
                            @endif
                            <x-buttons.icon
                                wire:click="$emitTo('admin.cash-closing.audit-modal', 'open', {{ $item->id }})"
